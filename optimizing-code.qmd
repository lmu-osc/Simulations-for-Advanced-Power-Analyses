...
timings$diff <- c(
  NA,
  timings[2:nrow(timings), 1] - timings[1:(nrow(timings) - 1), 1]
)
timings$rel_diff <- c(
  NA,
  timings[2:nrow(timings), "diff"] / timings[1:(nrow(timings) - 1), 1]
) |>
  round(3)
...
This change gave a small speed-up of `r timings[2, "diff"]` seconds (or `r  timings[2, "rel_diff"]*100`%).
...
This showed a substantial improvement of around `r round(t2[3] - t1[3], 1)` seconds; a relative gain of `r round(timings$rel_diff[3]*100, 1)`% (both compared to the preceding version).
...
t4 <- system.time({
  ...
  for (n in ns) {
    ...
    for (i in 1:iterations) {
      ...
      p_values[i] <- 2 *
        pt(
          abs(mdl$coefficients[2] / mdl$stderr[2]),
          mdl$df.residual,
          lower.tail = FALSE
        )
    }
...
This step made no visible increase in speed -- but as a bonus, it made our code much easier to read and shorter.
...
timings <- rbind(t0[3], t1[3], t2[3], t3[3], t4[3], t5[3], t6[3]) |>
  data.frame()
timings$diff <- c(
  NA,
  timings[2:nrow(timings), 1] - timings[1:(nrow(timings) - 1), 1]
...
Let's see if parallel processing has an advantage when we have longer computations. We now expand the simulation by exploring a broad parameter range (n ranging from 100 to 1000) and increasing the iterations to 20,000 for more stable results. (See also: "[Bonus: How many Monte Carlo iterations are necessary?](how-many-iterations.qmd)")
...
Some steps, such as avoiding growing vectors, didn't help much in our current example, but will help a lot in other scenarios.
...
These speed measurements have been performed on a 2025 MacBook Pro with M4 processor.
