...
aliases: [how_many_iterations.html]
---
...
To find the sample size needed for a study, we have previously used, say, 1000 iterations of data simulation and analysis, and varied the sample size *n* from, say, 100 to 1000, every 50, to find where the 80% power threshold was crossed (see [Chapter 1](lm1.qmd#sample-size-planning-find-the-necessary-sample-size)). But is 1000 iterations giving a precise enough result?

Using the [optimized code](optimizing-code.qmd), we can explore how many Monte Carlo iterations are necessary to get stable computational results: We can re-run the same simulation with the same sample size and see how much random variation is between results!
