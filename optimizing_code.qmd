...
for (n in ns) {
  ...
  result <- rbind(result, data.frame(n = n, power = sum(p_values < .005)/iterations)) #FIXME the result is printed in the preview, but it doesn't show what I'd expect from this line, it isn't two columns with n and power but only the values of n.
}
