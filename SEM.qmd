...
#install.packages(c("lavaan", "ggplot2", "Rfast", "apaTables", "MBESS", "future.apply"), dependencies = TRUE)
...
for (n in ns_sem) {  
  ...
  result_sem_safeguard <- rbind(result_sem_safeguard, data.frame(
    n = n,
    power = sum(p_values_sem_safeguard < .005)/iterations_sem)
  )
}
...
```{r means and cov mediation, include = FALSE}
