...
sim_sem <- function(n, model, mu, sigma) {
  ...
  rmsea_ci_width <- as.numeric(
    fitMeasures(fit_sem_simulated)["rmsea.ci.upper"] -
      fitMeasures(fit_sem_simulated)["rmsea.ci.lower"]
  )
...
for (n in ns) {
  rmsea_ci_width <- future_replicate(
    iterations,
